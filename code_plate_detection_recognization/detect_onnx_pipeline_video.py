...
def detect_onnx(save_img=True, save_conf=True):
    ...
    video_path = "/code/zzg/project/study/code_plate_detection_recognization/demo/video/车牌检测识别视频demo.mp4"
    output_dir = "/code/zzg/project/study/code_plate_detection_recognization/demo/video/vis_video_0.avi"
    ...
    while  have_more_frame:
        image_ori = frame
        frame_id += 1
        print("---", frame_id, image_ori.shape)
        ...
        for i, det in enumerate(pred):  # detections per image
            if len(det):
                ...
                image_vis = plot_one_box_class(xyxy_, image_ori, label=label, predstr=cls,
                                    line_thickness=3)
                print("===", image_ori.shape, cls)
                cv2.imwrite("0227.jpg", image_vis)
                # import pdb; pdb.set_trace()
