...
source = "/home/month_2_dadao/lesson4/code_plate_detection_recognization/demo/images"
video_pth = "/home/month_2_dadao/lesson4/code_plate_detection_recognization/demo/video/车牌检测识别视频demo.mp4"
det_model_path = "/home/month_2_dadao/lesson4/code_plate_detection_recognization/weights/yolov5.tflite"
recog_model_path = "/home/month_2_dadao/lesson4/code_plate_detection_recognization/weights/LPRNet_Simplified.tflite"
save_dir = "/home/month_2_dadao/lesson4/code_plate_detection_recognization/demo/output"
...
have_more_frame, frame = cap.read()
frame_id = 0
...
while have_more_frame:
    ...
    # 数据维度转换
        if pred.shape[0] == 28800:
           ...
           outputs[1] = pred           
        if pred.shape[0] == 115200: #7200
           pred = pred.reshape(1, 3,80,80, 6) #(1, 3,20,20, 6)
           outputs[0] = pred
        if pred.shape[0] == 7200: #115200
           pred = pred.reshape(1, 3,20,20, 6) #(1, 3,80,80, 6)
           outputs[2] = pred
    # 模型推理后处理
    boxes, confs, classes = det_poseprocess(outputs, imgsz, scale, left, top, conf_thresh=0.4, iou_thresh=0.5)   
    ...
    if len(pred) == 0:
        ...
        for i, det in enumerate(pred):  # detections per image
            if len(det):
                ...
                if (xyxy_[2]-xyxy_[0])/(xyxy_[3]-xyxy_[1])>6 or (xyxy_[2] -xyxy_[0])<50:
                    continue
                ...
                if conf > 0.65:
                    image_vis = plot_one_box_class(xyxy_, image_ori, label=label, predstr=cls,
                                        line_thickness=3)
                    cvs.imshow(image_vis)
                else:
                    cvs.imshow(image_ori)
            else:
                cvs.imshow(image_ori)
