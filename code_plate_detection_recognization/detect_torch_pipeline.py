...
def run(weights='weights/yolov5s.pt',  # 权重文件地址 默认 weights/yolov5.pt
    ...
    modelr = LPRNet(lpr_max_len= 8, phase=False, class_num=len(CHARS), dropout_rate=0)
    modelr.load_state_dict(torch.load('weights/lprnet_best.pth', map_location="cpu"))
    modelr.eval()
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    xyxy_ = np.copy(xyxy_).tolist()[0]
                    xyxy_ = [int(i) for i in xyxy_]
                    
                    image_clas = reg_preprocess(xyxy_, im0)
                    image_clas = torch.unsqueeze(torch.tensor(image_clas),dim=0)

                    probs = modelr(image_clas)
                    ...
                    if save_img or save_crop or view_img:
                        ...
                        # 画出预测框
                        plot_one_box(xyxy, im0, label=label, color=colors(1, True), line_thickness=3)
                        if save_crop:
...
def parse_opt():
    ...
    parser.add_argument('--weights', nargs='+', type=str, default="/home/leinao/zhangqinwei/codes/project/code_plate_detection_recognization/weights/yolov5_best.pt", help='model.pt path(s)')
    parser.add_argument('--recog_weights', nargs='+', type=str, default="/home/leinao/zhangqinwei/codes/project/code_plate_detection_recognization/weights/lprnet_best.pth", help='model.pt path(s)')
    parser.add_argument('--source', type=str, default="/home/leinao/zhangqinwei/codes/project/code_plate_detection_recognization/demo/images/", help='file/dir/URL/glob, 0 for webcam')
    ...
    parser.add_argument('--project', default=r'D:\code_plate_detection_recognization_1\detect', help='save results to project/name')
