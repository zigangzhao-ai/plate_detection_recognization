...
det_weights = "weights/yolov5_best.onnx"
clas_weights = "weights/LPRNet_Simplified.onnx"
...
def detect_onnx(save_img=True, save_conf=True):
    ...
    output_dir = "/code/zzg/project/study/code_plate_detection_recognization/demo/video/vis_video2.avi"
    writer = cv2.VideoWriter(output_dir, 
                cv2.VideoWriter_fourcc('M','J','P','G'), # type - avi
                15, 
                (368, 640))
    imgs_path = sorted(imgs_path, key=lambda x: int(x[5:-4])) ## name like "frame1.jpg, frame2.jpg"

    for img_pth in imgs_path:
        print(img_pth)
        image_ori = cv2.imread(os.path.join(source, img_pth))
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if save_img:
                new_img_path = os.path.join(save_dir, img_pth.split("/")[-1])
                cv2.imwrite(new_img_path, image_vis)
        else:      
            writer.write(image_ori)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--detect_weights', nargs='+', type=str,
                        default=r"weights/yolov5_best.onnx",
                        help='detection model path(s)')
    parser.add_argument('--classifi_weights', nargs='+', type=str,
                        default=r"weights/Final_LPRNet_Simplified.onnx",
                        help='classification model path(s)')
    parser.add_argument('--source', type=str, default=r"demo/images/1319",
                        help='source')  # file/folder, 0 for webcam
    parser.add_argument('--save_dir', type=str, default=r"demo/output2",
                        help='source')  # folder,
